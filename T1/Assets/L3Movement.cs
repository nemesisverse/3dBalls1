...
public class L3Movement : MonoBehaviour , IFallingBlock
{
    ...
    //  blockGuide is resolved at runtime via Find because L3Movement is a
    //  prefab instantiated during gameplay — a scene object cannot be
    //  pre-assigned in the prefab's Inspector field.
    //  l3MovementIndicators — the child names that belong to this block shape.
    // ================================================================
    private GameObject blockGuide;
    private static readonly string[] l3MovementIndicators = { "(1,2)", "(2,2)", "(3,2)", "(1,1)" };
    ...
    void SetIndicators()
    {
        ...
        // Activate only the cells that belong to L3Movement
        foreach (string indicatorName in l3MovementIndicators)
        {
    ...
    //
    //  Left diagonal (spots 1-3): active ring checks → if (!Check...) wrapper.
    //  Uses the Z-style -2/-1 offset (block at [i-2], flagRadius at
    //  [i-2] for blocked / [i-1] for end-of-track).
    //
    //  Vertical (spots 1-3, as labelled in the original): no ring check
    //  and no TryDestroySelf exist anywhere in moveVertical — not even
    //  commented out. CheckGameOverOnLanding is called standalone only.
    // ================================================================
    ...
    IEnumerator moveLeftDiagonal(Transform child, int childCount)
    {
        ...
        if (childCount == 1)
        {
            ...
            {
                // Pre-move look-ahead: lead block's upcoming position is [indexCountLeft-1].
                if (stop == -1)
                {
                    bool blocked = false;
