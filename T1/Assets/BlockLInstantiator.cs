...
public class BlockLInstantiator : MonoBehaviour
{
    ...
    private void HandleTap(Vector2 screenPosition)
    {
        ...
        if (logSpawnInfo)
            Debug.Log($"[BlockLInstantiator] Tap → trying candidate: {candidateType}");

        GameObject prefab = PrefabForType(candidateType);
        if (prefab == null)
        {
            Debug.LogError($"[BlockLInstantiator] Prefab for {candidateType} not assigned.");
            return;
        }
        ...
        if (CheckCandidateOverlap(candidate))
        {
            ...
            if (logSpawnInfo)
                Debug.Log($"[BlockLInstantiator] Candidate {candidateType} BLOCKED — keeping current block.");
        ...
        if (logSpawnInfo)
            Debug.Log($"[BlockLInstantiator] Candidate {candidateType} CLEAR — committing swap.");
