...
public class BlockEyeInstantiator : MonoBehaviour
{
    ...
    private void HandleTap(Vector2 screenPosition)
    {
        ...
        if (logSpawnInfo)
            Debug.Log($"[BlockEyeInstantiator] Tap → trying candidate: {candidateType}");

        GameObject prefab = PrefabForType(candidateType);
        if (prefab == null)
        {
            Debug.LogError($"[BlockEyeInstantiator] Prefab for {candidateType} not assigned.");
            return;
        }
        ...
        if (CheckCandidateOverlap(candidate))
        {
            ...
            if (logSpawnInfo)
                Debug.Log($"[BlockEyeInstantiator] Candidate {candidateType} BLOCKED — keeping current block.");
        ...
        if (logSpawnInfo)
            Debug.Log($"[BlockEyeInstantiator] Candidate {candidateType} CLEAR — committing swap.");
