...
public class S1Movement : MonoBehaviour, IFallingBlock
{
    ...
    //  blockGuide is resolved at runtime via Find because S1Movement is a
    //  prefab instantiated during gameplay — a scene object cannot be
    //  pre-assigned in the prefab's Inspector field.
    //  s1MovementIndicators — the child names that belong to this block shape.
    // ================================================================
    private GameObject blockGuide;
    private static readonly string[] s1MovementIndicators = { "(2,1)", "(3,1)", "(2,3)", "(3,3)" };
    ...
    void SetIndicators()
    {
        ...
        // Activate only the cells that belong to S1Movement
        foreach (string indicatorName in s1MovementIndicators)
        {
